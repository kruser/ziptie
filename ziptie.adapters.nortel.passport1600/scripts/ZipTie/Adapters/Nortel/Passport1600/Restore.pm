package ZipTie::Adapters::Nortel::Passport1600::Restore;
...
use ZipTie::Adapters::Nortel::Passport1600::Disconnect
	qw(disconnect);
use ZipTie::Logger;
...
sub _connect
{
	...
	my $device_prompt_regex = ZipTie::Adapters::Nortel::Passport1600::AutoLogin::execute( $cli_protocol, $connection_path );
...
sub execute
{
	...
	{
		$LOGGER->fatal("Unable to restore Passport1600 config. Protocol TFTP is not available.");
	}
}
...
sub restore_via_tftp
{
	...
	my @responses = ();
	push(@responses, ZipTie::Response->new('File not found|TFTP Establish session fail|[Ee]rror|[Ff]ail', undef, $TFTP_ERROR));
	push(@responses, ZipTie::Response->new('Done\. Press ENTER to continue\.', \&_finish));

	$cliProtocol->send( "download configuration ".$tftpFileServer->get_ip_address()." $configName" );
...
sub _finish
{
	...
	$cliProtocol->send_and_wait_for( "", $promptRegex );
