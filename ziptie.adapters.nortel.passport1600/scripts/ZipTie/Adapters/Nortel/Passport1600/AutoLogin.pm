package ZipTie::Adapters::Nortel::Passport1600::AutoLogin;
...
sub _initial_connection
{
	...
	push( @responses, ZipTie::Response->new( '3#\s*$', \&_send_enable ) );
	push( @responses, ZipTie::Response->new( '4#\s*$', \&_get_prompt ) );
...
sub _send_enable
{
	...
	{    
		# Specify the responses to check for
		my @responses = ();
		push( @responses, ZipTie::Response->new( 'Fail!', undef, $INVALID_CREDENTIALS ) );
		push( @responses, ZipTie::Response->new( '[Pp]assword:', \&_send_enable_password ) );
		push( @responses, ZipTie::Response->new( '4#\s*$',   \&_get_prompt ) );

		# Send enable
		$cli_protocol->send('enable admin');
		my $response = $cli_protocol->wait_for_responses( \@responses );
...
sub _send_enable_password
{
	...
	# Specify the responses to check for
	my @responses = ();
	push( @responses, ZipTie::Response->new( '[Pp]assword:', undef, $INVALID_CREDENTIALS ) );
	push( @responses, ZipTie::Response->new( '4#\s*$', \&_get_prompt ) );
