package ZipTie::Adapters::Nokia::Checkpoint::AutoLogin;
...
sub _initial_connection
{
	...
	push(@responses, ZipTie::Response->new('Terminal\s+type\?', \&_send_term_type));
...
sub _send_term_type
{
	...
	# Send username credential
	my $term_type = 'vt100';
	$LOGGER->debug("Sending terminal type ...");
	$cli_protocol->send($term_type);
	my $response = $cli_protocol->wait_for_responses(\@responses);
...
sub _get_prompt
{
	...
	# Send a blank line in order to get a clean response
	$LOGGER->debug("Sending an empty string to get a clean response from the device");
	$cli_protocol->send_and_wait_for("", '(#|\$)\s*$');
...
ZipTie::Adapters::Nokia::Checkpoint::AutoLogin - An example autologin module

=head1 SYNOPSIS

    use ZipTie::Adapters::Nokia::Checkpoint;
	my $cli_protocol = ZipTie::CLIProtocolFactory::create( $connection_path );
	my $prompt_regex = ZipTie::Adapters::Nokia::Checkpoint::execute( $cli_protocol, $connection_path );
