package ZipTie::Adapters::Juniper::ScreenOS::AutoLogin;
...
sub _send_password
{
	...
	# Specify the responses to check for
	my @responses = ();
	push(@responses, ZipTie::Response->new('incorrect|invalid|Login failed', undef, $INVALID_CREDENTIALS));
	push(@responses, ZipTie::Response->new('(#|>)\s*$', \&_get_prompt));
...
ZipTie::Adapters::Juniper::ScreenOS::AutoLogin - An example autologin module

=head1 SYNOPSIS

    use ZipTie::Adapters::Juniper::ScreenOS;
	my $cli_protocol = ZipTie::CLIProtocolFactory::create( $connection_path );
	my $prompt_regex = ZipTie::Adapters::Juniper::ScreenOS::execute( $cli_protocol, $connection_path );
