package ZipTie::Adapters::Cisco::Three005::AutoLogin;
...
sub _initial_connection
{
	...
	push(@responses, ZipTie::Response->new('->', \&_get_prompt));
...
sub _send_password
{
	...
	# Specify the responses to check for
	my @responses = ();
	push(@responses, ZipTie::Response->new('denied', undef, $INVALID_CREDENTIALS));
	push(@responses, ZipTie::Response->new('nvalid', undef, $INVALID_CREDENTIALS));
	push(@responses, ZipTie::Response->new('->', \&_get_prompt));
...
ZipTie::Adapters::Cisco::Three005::AutoLogin - An example autologin module

=head1 SYNOPSIS

    use ZipTie::Adapters::Cisco::Three005;
	my $cli_protocol = ZipTie::CLIProtocolFactory::create( $connection_path );
	my $prompt_regex = ZipTie::Adapters::Cisco::Three005::execute( $cli_protocol, $connection_path );
