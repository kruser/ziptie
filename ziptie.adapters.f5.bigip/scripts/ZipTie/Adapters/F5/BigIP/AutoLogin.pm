package ZipTie::Adapters::F5::BigIP::AutoLogin;
...
sub execute
{
	...
	# If all goes well during the login process, the last method to be called will be "_get_prompt_regex"
...
sub _initial_connection
{
	...
	#push(@responses, ZipTie::Response->new('continue connecting \(yes\/no\)\?\s*$', \&_send_confirmation));
...
ZipTie::Adapters::F5::BigIP::AutoLogin - Automates the connection and authentication with a F5 BigIP device.

=head1 SYNOPSIS

    use ZipTie::Adapters::F5::BigIP;
	my $cli_protocol = ZipTie::CLIProtocolFactory::create( $connection_path );
	my $prompt_regex = ZipTie::Adapters::F5::BigIP::execute( $cli_protocol, $connection_path );
