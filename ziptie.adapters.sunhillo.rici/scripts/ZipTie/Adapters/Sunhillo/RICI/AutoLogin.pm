package ZipTie::Adapters::Sunhillo::RICI::AutoLogin;
...
sub execute
{
	...
	if ( $prompt_regex !~ /root/ )
	{
		$prompt_regex = _su( $cli_protocol, $credentials );
	}
...
sub _send_enable_password
{
	...
	# Send enable password credential
	my $password = $credentials->{enablePassword};
	$LOGGER->debug("Sending enable password credential for su access...");
	$cli_protocol->send($password);
	my $response = $cli_protocol->wait_for_responses( \@responses );
