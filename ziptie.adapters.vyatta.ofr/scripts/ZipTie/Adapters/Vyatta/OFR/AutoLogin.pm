package ZipTie::Adapters::Vyatta::OFR::AutoLogin;
...
sub _initial_connection
{
	...
	my @responses = ();
	push( @responses, ZipTie::Response->new( '(?<!Last)\s+login:\s*$', \&_send_username ) );
	push( @responses, ZipTie::Response->new( 'assword:', \&_send_password ) );
	push( @responses, ZipTie::Response->new( '.+~#\s+$',  \&_enter_xorpsh ) );
...
sub _send_password
{
	...
	# Specify the responses to check for
	my @responses = ();
	push( @responses, ZipTie::Response->new( 'ogin incorrect', undef, $INVALID_CREDENTIALS ) );
	push( @responses, ZipTie::Response->new( '.+~#\s+$', \&_enter_xorpsh ) );
	push( @responses, ZipTie::Response->new( '.+>', \&_calculate_prompt_regex ) );
...
sub _enter_xorpsh
{
	...
	# Specify the responses to check for
	my @responses = ();
	push( @responses, ZipTie::Response->new( 'command not found', undef, $INSUFFICIENT_PRIVILEGE ) );
	push( @responses, ZipTie::Response->new( '.+>', \&_calculate_prompt_regex ) );

	# Send the "xorpsh" command to enter the OFR CLI
	$LOGGER->debug("Entering OFR CLI ...");
	$cli_protocol->send("xorpsh");
	my $response = $cli_protocol->wait_for_responses( \@responses );
...
ZipTie::Adapters::Vyatta::OFR::AutoLogin - Automates the connection and authentication with a Vyatta Open Flexible Router (OFR) device.

=head1 SYNOPSIS

    use ZipTie::Adapters::Vyatta::OFR::AutoLogin;
	my $cli_protocol = ZipTie::CLIProtocolFactory::create( $connection_path );
	my $prompt_regex = ZipTie::Adapters::Vyatta::OFR::AutoLogin::execute( $cli_protocol, $connection_path );
...
=item C<_enter_xorpsh($cli_protocol, $connection_path)>

This sub-routine is called if a response from the OFR device indicates that we are in a bash-like shell.  The users
of this auto-login module assume that C<ZipTie::CLIProtocol> object is interfacing with the XORP shell on the OFR.
This means that the "xorpsh" command must be sent to the device to move from the bash-style shell to the XORP shell.
