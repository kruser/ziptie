package ZipTie::Adapters::Cisco::IOS::GetRunningConfig;
...
sub get_running_config
{
	...
	# Check to see if SCP is supported.  If so, a SCP client will be used to retrieve the running configuration
	elsif ( defined($scp_protocol) )
	{
		$response = _get_running_config_scp($connection_path);
	}
...
sub _get_running_config_cli
{
	...
	elsif ($response =~ /Unable to get configuration/i)
	{
		$LOGGER->fatal_error_code($NVRAM_CORRUPTION_ERROR, $cli_protocol->get_ip_address(), "Unable to issue \"$command\"");
	}
	# remove the prompt
	$response =~ s/$regex$//;
...
sub _get_running_config_scp
{
	...
	# For IOS-based devices, the running configuration is stored with in the "running-config" file
	my $running_config_name = "running-config";
	...
	$xfer_client->get( $running_config_name, $temp_filename );

	# Open up the running configuration file and read it into memory
	open(RUNNING_CONFIG, $temp_filename) || $LOGGER->fatal("[$SCP_ERROR]\nCould not open the retrieved running configuration file stored in '$temp_filename'");
    ...
    $RECORDING->create_xfer_interaction($scp_protocol->get_name(), $running_config_name, $running_config_contents, 0);
...
sub _get_running_config_tftp
{
	...
	# Construct a possible response scenario where the "copy running-config tftp" command is not supported.
	# If we encounter this response, the command wasn't valid.  Let's try the "copy nvram tftp://" command
...
sub _copy_nvram()
{
	...
	push(@responses, ZipTie::Response->new('Invalid input', undef, $INSUFFICIENT_PRIVILEGE));
