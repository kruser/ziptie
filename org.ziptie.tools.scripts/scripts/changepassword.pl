...
my ( $connectionPathXml, $adapterId, $username, $newPassword );
GetOptions(
...
my $passwordSettings	= '<username>' . $username . '</username>';
$passwordSettings		.= '<newPassword>' . $newPassword . '</newPassword>';
my $operation       	= 'changePassword';
