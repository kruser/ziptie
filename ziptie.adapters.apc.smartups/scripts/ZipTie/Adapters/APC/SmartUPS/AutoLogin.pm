package ZipTie::Adapters::APC::SmartUPS::AutoLogin;
...
sub _initial_connection
{
	...
	my @responses = ();
	push(@responses, ZipTie::Response->new('User Name\s*:|(?<!Last)\s+login:', \&_send_username));
	push(@responses, ZipTie::Response->new('[Pp]assword\s*:', \&_send_password));
...
sub _send_password
{
	...
	# Specify the responses to check for
	my @responses = ();
	push(@responses, ZipTie::Response->new('incorrect|login invalid|User Name\s*:', undef, $INVALID_CREDENTIALS));
	push(@responses, ZipTie::Response->new('(#|\$|>)\s*$', \&_get_prompt));
