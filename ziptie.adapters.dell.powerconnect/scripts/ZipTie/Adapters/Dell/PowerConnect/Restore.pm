package ZipTie::Adapters::Dell::PowerConnect::Restore;
...
use ZipTie::Adapters::Dell::PowerConnect::Disconnect
	qw(disconnect);
use ZipTie::Logger;
...
sub restore_via_tftp
{
	...
	push(@responses, ZipTie::Response->new('The copy operation was completed successfully', \&_finish));

	$cliProtocol->send( "copy tftp://".$tftpFileServer->get_ip_address()."/$configName ".$promoteFile->get_path() );
