package ZipTie::Adapters::Dell::PowerConnect::AutoLogin;
...
sub _initial_connection
{
	...
	my @responses = ();
	push( @responses, ZipTie::Response->new( '[Uu]ser [Nn]ame:', \&_send_username ) );
...
sub _send_password
{
	...
	# Specify the responses to check for
	my @responses = ();
	push( @responses, ZipTie::Response->new( '[Uu]ser [Nn]ame:',          undef, $INVALID_CREDENTIALS ) );
...
sub _send_enable_password
{
	...
	# Specify the responses to check for
	my @responses = ();
	push( @responses, ZipTie::Response->new( '[Pp]assword:|incorrect|invalid', undef, $INVALID_CREDENTIALS ) );
	push( @responses, ZipTie::Response->new( '(#|\$)\s*$', \&_get_prompt ) );
...
ZipTie::Adapters::Cisco::SecurityAppliance::AutoLogin - Automates the connection and authentication with a
...
=head1 SYNOPSIS
	...
	my $prompt_regex = ZipTie::Adapters::Cisco::SecurityAppliance::AutoLogin::execute( $cli_protocol, $connection_path );
