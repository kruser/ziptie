package ZipTie::Adapters::Alteon::AD3::AutoLogin;
...
sub _initial_connection
{
	...
	push(@responses, ZipTie::Response->new('Confirm seeing above note', \&_send_yes));
...
sub _send_yes
{
	...
	# Send username credential
	$LOGGER->debug("Sending y to confirm seeing note ...");
	$cli_protocol->send("y");
	my $response = $cli_protocol->wait_for_responses(\@responses);
