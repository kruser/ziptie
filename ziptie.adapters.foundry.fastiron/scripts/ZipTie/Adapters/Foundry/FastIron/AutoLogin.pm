package ZipTie::Adapters::Foundry::FastIron::AutoLogin;
...
sub _initial_connection
{
	...
	my @responses = ();
	push( @responses, ZipTie::Response->new( 'number of telnet', undef, $TOO_MANY_USERS ) );
	push( @responses, ZipTie::Response->new( '.*', \&_initial_prompt ) );
...
sub _initial_prompt
{
	...
	push( @responses, ZipTie::Response->new( 'Username:',    \&_send_username ) );
	push( @responses, ZipTie::Response->new( 'Login Name',   \&_send_username ) );
	push( @responses, ZipTie::Response->new( 'Password:',    \&_send_password ) );
	push( @responses, ZipTie::Response->new( '\S+@.+\>\s*$', \&_send_enable ) );
...
sub _send_password
{
	...
	# Specify the responses to check for
	my @responses = ();
	push( @responses, ZipTie::Response->new( 'incorrect|invalid|login failure', undef, $INVALID_CREDENTIALS ) );
	push( @responses, ZipTie::Response->new( '\S+@.+\>\s*$', \&_send_enable ) );
...
sub _send_enable
{
	...
	if ( $credentials->{enablePassword} )
	{
		...
		# Specify the responses to check for
		my @responses = ();
		push( @responses, ZipTie::Response->new( 'User Name:',  \&_send_enable_username ) );
		push( @responses, ZipTie::Response->new( 'Password:',   \&_send_enable_password ) );
		push( @responses, ZipTie::Response->new( '\S+@.+#\s*$', \&_calculatePromptRegEx ) );
...
sub _send_enable_password
{
	...
	# Specify the responses to check for
	my @responses = ();
	push( @responses, ZipTie::Response->new( '[Ii]ncorrect|login failure|\S+@.+>\s*$', undef, $INVALID_CREDENTIALS ) );
	push( @responses, ZipTie::Response->new( '\S+@.+#\s*$', \&_calculatePromptRegEx ) );
...
ZipTie::Adapters::Foundry::FastIron::AutoLogin - Automates the connection and authentication with a Cisco IOS device.

=head1 SYNOPSIS

    use ZipTie::Adapters::Foundry::FastIron::AutoLogin;
	my $cli_protocol = ZipTie::CLIProtocolFactory::create( $connection_path );
	my $prompt_regex = ZipTie::Adapters::Foundry::FastIron::AutoLogin::execute( $cli_protocol, $connection_path );
