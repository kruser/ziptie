package ZipTie::Adapters::HP::ProCurve::GetStartupConfig;
...
sub _get_startup_config_cli
{
	...
	if ( $response =~ /Startup configuration:(.+?)$regex/mis )
	{
		$response = $1;
	}
...
sub _get_startup_config_tftp
{
	...
	# Sending "copy startup-config tftp <IP> <FILE>" command
	...
	my $command = "copy startup-config tftp " . $tftp_ip_address . " " . $startupConfigName;
	$cli_protocol->send( $command );
