package ZipTie::Adapters::Aruba::ArubaOS::Restore;
...
sub execute
{
	...
	if ( $restoreFile->get_path() =~ /config/i )
	{
		...
		{
			$LOGGER->fatal("Unable to restore Aruba config. Protocol TFTP is not available.");
		}
	}
	else
	{
...
sub restore_via_tftp
{
	...
	$cliProtocol->send( "copy tftp: " . $tftpFileServer->get_ip_address() . " $configName flash: " . $promoteFile->get_path() );
...
sub _finish
{
	...
	$cliProtocol->send_and_wait_for( 'boot config-file '.$promoteFile->get_path(), $promptRegex );
