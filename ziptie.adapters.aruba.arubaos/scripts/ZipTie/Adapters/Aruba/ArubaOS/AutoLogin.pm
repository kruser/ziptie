package ZipTie::Adapters::Aruba::ArubaOS::AutoLogin;
...
sub _initial_connection
{
	...
	my @responses = ();
	push( @responses, ZipTie::Response->new('[Uu]ser:', \&_send_username) );
...
sub _send_username
{
	...
	my @responses = ();
	push(@responses, ZipTie::Response->new('[Pp]assword:', \&_send_password));
	push(@responses, ZipTie::Response->new('incorrect|invalid|WARNING|User:', undef, $INVALID_CREDENTIALS));
...
sub _send_enable
{
	...
	{    
		# Specify the responses to check for
		my @responses = ();
		push( @responses, ZipTie::Response->new( 'incorrect|invalid|WARNING', undef, $INVALID_CREDENTIALS ) );
...
ZipTie::Adapters::Aruba::ArubaOS::AutoLogin - An example autologin module

=head1 SYNOPSIS

    use ZipTie::Adapters::Aruba::ArubaOS;
	my $cli_protocol = ZipTie::CLIProtocolFactory::create( $connection_path );
	my $prompt_regex = ZipTie::Adapters::Aruba::ArubaOS::execute( $cli_protocol, $connection_path );
