package ZipTie::Adapters::Extreme::Switch::AutoLogin;
...
sub _initial_connection
{
	...
	push(@responses, ZipTie::Response->new('(#|\$)\s*$', \&_disable_cli_paging));
...
sub _disable_cli_paging
{
	...
	$cli_protocol->send('disable clipaging');
	my $response = $cli_protocol->wait_for_responses(\@responses);
...
sub _get_prompt
{
	...
	# Changing prompt regex it comes with a static number ":1" should be :\d+  in --> /\*\ MIA\-Extreme300\:1\ \#\s*$/
    $device_prompt_regex =~ s/:\d+/:\\d+/i;
...
ZipTie::Adapters::Extreme::Switch::AutoLogin - An example autologin module

=head1 SYNOPSIS

    use ZipTie::Adapters::Extreme::Switch;
	my $cli_protocol = ZipTie::CLIProtocolFactory::create( $connection_path );
	my $prompt_regex = ZipTie::Adapters::Extreme::Switch::execute( $cli_protocol, $connection_path );
