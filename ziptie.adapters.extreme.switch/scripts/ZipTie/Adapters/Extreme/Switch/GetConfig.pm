package ZipTie::Adapters::Extreme::Switch::GetConfig;
...
sub _get_config_cli
{
	...
	my $command = "show configuration detail";
	$cli_protocol->send( $command );
	
	my $response = $cli_protocol->wait_for($regex, 180);
	
	# remove the prompt
	$response =~ s/^$command//;
	$response =~ s/Configuration generated.+$//m;
	$response =~ s/$regex$//;
	
	# remove any --More-- prompt lines from older (1900) devices
	$response =~ s/^Press\s+<Space>\s+.+:$//mig;
...
sub _get_config_tftp
{
	...
	my @responses = ();
	push(@responses, ZipTie::Response->new($regex,\&_finish));
	push(@responses, ZipTie::Response->new('(?i:ERROR)', undef, $TFTP_ERROR));
	
	#Ex: Sending "upload configuration 192.168.11.203 config" command
	my $command = "upload configuration $tftp_ip_address $config_name";
	$cli_protocol->send( $command );
...
sub _finish
{
	...
	#Some OS's or TFTP servers might be too slow, 
	#and take a little longer to unlock the file, so we wait for those
	sleep (10);
