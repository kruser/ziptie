package ZipTie::Adapters::Alcatel::Omniswitch::Parsers;
...
sub parse_chassis
{
	...
	$chassis->{'core:asset'}->{'core:factoryinfo'}->{'core:make'} = 'Alcatel';
	if ( $in->{info} =~ /System Type:\s*(\b.+\b)/ )
	{
		$chassis->{'core:asset'}->{'core:factoryinfo'}->{'core:modelNumber'} = $1;
	}

	# $chassis->{'core:asset'}->{'core:factoryinfo'}->{'core:serialNumber'} = 'TBD';
	if ( $in->{info} =~ /Description:\s*(\b.+\b)/ )
	{
		$chassis->{'core:description'} = $1;    
	}
...
sub parse_system
{
	...
	$out->print_element( 'core:make',    'Alcatel' );
	if($in->{slot} =~ /\s+(\d+\.\d+\.\d+)\s+/)
	{
		$out->print_element( 'core:version', $1 );
	}
	$out->print_element( 'core:osType',  'OmniSwitch' );
