package ZipTie::Adapters::Nortel::Passport::AutoLogin;
...
sub _send_password
{
	...
	# Specify the responses to check for
	my @responses = ();
	push(@responses, ZipTie::Response->new('incorrect|invalid|Login:', undef, $INVALID_CREDENTIALS));
...
sub _send_enable
{
	...
	# Specify the responses to check for
	my @responses = ();
    push(@responses, ZipTie::Response->new('[pP]assword:', \&_send_enable_pass));
    push(@responses, ZipTie::Response->new('(#|\$)\s*$', \&_get_prompt));
...
sub _send_enable_pass
{
	...
	# Specify the responses to check for
	my @responses = ();
	push(@responses, ZipTie::Response->new('assword|incorrect|invalid', undef, $INVALID_CREDENTIALS));
    push(@responses, ZipTie::Response->new('(#|\$)\s*$', \&_get_prompt));
	
	# Send enable pass credential
	my $password = $credentials->{enablePassword};
	$LOGGER->debug("Sending enablePassword credential ...");
	$cli_protocol->send($password);
	my $response = $cli_protocol->wait_for_responses(\@responses);
...
ZipTie::Adapters::Nortel::Passport::AutoLogin - An example autologin module

=head1 SYNOPSIS

    use ZipTie::Adapters::Nortel::Passport;
	my $cli_protocol = ZipTie::CLIProtocolFactory::create( $connection_path );
	my $prompt_regex = ZipTie::Adapters::Nortel::Passport::execute( $cli_protocol, $connection_path );
