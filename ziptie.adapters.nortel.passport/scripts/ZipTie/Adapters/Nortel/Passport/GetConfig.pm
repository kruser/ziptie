package ZipTie::Adapters::Nortel::Passport::GetConfig;
...
sub _get_config_tftp
{
	...
	my @responses = ();
	push(@responses, ZipTie::Response->new('Can\'t\s+open', undef, $TFTP_ERROR));
	push(@responses, ZipTie::Response->new('>|#', \&_finish));

	# Sending "copy /flash/$configfile LocalHostIP:$configfile" command
	my ($cfgFileShort)	= $filename =~ /\/([^\/]+)$/;
	my $cfgFile			= escape_filename($cli_protocol->get_ip_address().".".$cfgFileShort);
	create_empty_file( $tftp_file_server->get_root_dir() . '/' . $cfgFile );
	my $command			= "copy $filename ".$tftp_file_server->get_ip_address().":$cfgFile";
	$cli_protocol->send( $command );
...
sub _finish
{
	...
	my ($cfgFileShort)	= $filename =~ /\/([^\/]+)$/;
	my $config_name = $cli_protocol->get_ip_address() . ".$cfgFileShort";
