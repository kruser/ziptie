package ZipTie::Adapters::Enterasys::SSR::Restore;
...
use ZipTie::Adapters::Enterasys::SSR::Disconnect
	qw(disconnect);
use ZipTie::Logger;
...
sub _connect
{
	...
	my $device_prompt_regex = ZipTie::Adapters::Enterasys::SSR::AutoLogin::execute( $cli_protocol, $connection_path );
...
sub execute
{
	...
	if ( $restoreFile->get_path() =~ /^startup$/i )
	{
		...
		{
			$LOGGER->fatal("Unable to restore SSR config. Protocol TFTP is not available.");
		}
	}
	else
	{
...
sub restore_via_tftp
{
	...
	my @responses = ();
	push(@responses, ZipTie::Response->new('[^\s-]+-E\S+', undef, $TFTP_ERROR));
	push(@responses, ZipTie::Response->new('overwrite the [Ss]tartup configuration', \&_confirm));
	push(@responses, ZipTie::Response->new('TFTP-I-XFERRATE', \&_finish));

	$cliProtocol->send( "copy tftp://" . $tftpFileServer->get_ip_address() . "/$configName to " . $promoteFile->get_path() );
