package ZipTie::Adapters::ThreeCom::Switch4400::GetConfig;
...
sub _get_config_tftp
{
	...
	#push(@responses, ZipTie::Response->new('Save of system configuration successful', \&_finish));
	push(@responses, ZipTie::Response->new('File Name', \&_specify_source_file));
	push(@responses, ZipTie::Response->new('Enter user notes:', \&_specify_download_note));
	push(@responses, ZipTie::Response->new('TFTP Server IP Address', \&_specify_tftp_address));

	# Sending "system backupConfig save ###.###.###.### ###.###.###.###.config note" command
	my $command = "system backupConfig save";
