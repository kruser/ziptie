package ZipTie::Adapters::Nortel::BayRS::AutoLogin;
...
my $BCC_FAILED = 'COULD NOT START BCC';
...
sub _initial_connection
{
	...
	push(@responses, ZipTie::Response->new('(#|\$)\s*$', \&_send_bcc));
	push(@responses, ZipTie::Response->new('bcc\s*>\s*$', \&_get_prompt));
...
sub _send_username
{
	...
	my @responses = ();
	push(@responses, ZipTie::Response->new('[Pp]assword:', \&_send_password));
	#push(@responses, ZipTie::Response->new('Nortel\s+Networks\,\s+Inc\.', undef, $INVALID_CREDENTIALS));
	#push(@responses, ZipTie::Response->new('Bay\s+Networks\,\s+Inc\.', undef, $INVALID_CREDENTIALS));
...
sub _send_password
{
	...
	# Specify the responses to check for
	my @responses = ();
	push(@responses, ZipTie::Response->new('sername:|ogin:', undef, $INVALID_CREDENTIALS));
	push(@responses, ZipTie::Response->new('Nortel\s+Networks\,\s+Inc\.', undef, $INVALID_CREDENTIALS));
	push(@responses, ZipTie::Response->new('Bay\s+Networks\,\s+Inc\.', undef, $INVALID_CREDENTIALS));
	push(@responses, ZipTie::Response->new('(#|\$)\s*$', \&_send_bcc));
	push(@responses, ZipTie::Response->new('bcc\s*>\s*$', \&_get_prompt));
...
sub _send_bcc
{
	...
	# Specify the responses to check for
	my @responses = ();
	push(@responses, ZipTie::Response->new('Unable to load bcc command from file|BCC\s+requires\s+\d+\s+Mb\s+of\s+free\s+memory|(?:#|\$)\s*$', undef, $BCC_FAILED));
	push(@responses, ZipTie::Response->new('bcc\s*>\s*$', \&_get_prompt));

	# Send username credential
	my $password = $credentials->{password};
	$LOGGER->debug("Sending bcc ...");
	$cli_protocol->send('bcc');
...
ZipTie::Adapters::Nortel::BayRS::AutoLogin - An example autologin module

=head1 SYNOPSIS

    use ZipTie::Adapters::Nortel::BayRS;
	my $cli_protocol = ZipTie::CLIProtocolFactory::create( $connection_path );
	my $prompt_regex = ZipTie::Adapters::Nortel::BayRS::execute( $cli_protocol, $connection_path );
