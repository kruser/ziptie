package ZipTie::Adapters::Nortel::BayRS::GetConfig;
...
sub _get_config_cli
{
	...
	$_ = $cli_protocol->send_and_wait_for( "show config", $prompt_regex );

	# Clean the output
	s/^show config\s*$//mig; # remove leading cruft from the 'show' command output
...
sub _get_config_tftp
{
	...
	# Output the configuration
	$_ = $cli_protocol->send_and_wait_for( "show dinfo", $prompt_regex );
	my $disk_num = 1;
	$disk_num = $1 if ( /^\s*(\d+):/m );
	...
	my @responses = ();
	push(@responses, ZipTie::Response->new('[Ee]rror|[Ff]ail|[Ii]nvalid', undef, $TFTP_ERROR));
	push(@responses, ZipTie::Response->new('completed successfully', \&_finish));

	my $filename = escape_filename ( $cli_protocol->get_ip_address() ) . ".config";
	...
	# Get remote file
	my $command = "tftp put ".$tftp_file_server->get_ip_address()." $disk_num:config $filename";
