package ZipTie::Adapters::ThreeCom::CoreBuilder::GetConfig;
...
sub _get_config_tftp
{
	...
	my @responses = ();
	push(@responses, ZipTie::Response->new('Error|Failed', undef, $TFTP_ERROR));
	push(@responses, ZipTie::Response->new('NV Control file', \&_specify_source_file));
	push(@responses, ZipTie::Response->new('Host IP address', \&_specify_tftp_address));
...
sub _specify_source_file()
{
	...
	my @responses = ();
	push(@responses, ZipTie::Response->new('Error|Failed', undef, $TFTP_ERROR));
	push(@responses, ZipTie::Response->new('Invalid pathname', undef, $TFTP_ERROR));
	push(@responses, ZipTie::Response->new('Host IP address', \&_specify_tftp_address));
	push(@responses, ZipTie::Response->new('Enter an optional file label', \&_specify_download_note));
...
sub _specify_tftp_address
{
	...
	push(@responses, ZipTie::Response->new('Error|Failed', undef, $TFTP_ERROR));
	push(@responses, ZipTie::Response->new('NV Control file', \&_specify_source_file));
	push(@responses, ZipTie::Response->new('TFTP Server IP Address', \&_specify_tftp_address));
...
sub _specify_download_note
{
	...
	my @responses = ();
	push(@responses, ZipTie::Response->new('Error|Failed', undef, $TFTP_ERROR));
	push(@responses, ZipTie::Response->new('System NV data successfully stored', \&_finish));

	# Sending "y" to confirm
	$cli_protocol->send( "ZipTie Backup" );
